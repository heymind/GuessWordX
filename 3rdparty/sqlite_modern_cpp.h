...
namespace sqlite {
    ...
    private:
        ...
        template<typename T, typename A>
        friend void get_col_from_db(database_binder &db, int inx, std::vector<T, A> &val);

        /* for nullptr & unique_ptr support */
        friend database_binder &operator<<(database_binder &db, std::nullptr_t);

        template<typename T>
        friend database_binder &operator<<(database_binder &db, const std::unique_ptr<T> &val);

        template<typename T>
        friend void get_col_from_db(database_binder &db, int inx, std::unique_ptr<T> &val);
    ...
    public:
        ...
        void operator>>(std::vector<std::vector<std::string>> &values) {
            int col_count = sqlite3_column_count(_stmt.get());
            this->_extract([&values, this, col_count] {
                std::vector<std::string> row(col_count);
                for (int i = 0; i < col_count; i++) {
                    get_col_from_db(*this, i, row.at(i));
                }
    ...
    public:
        ...
        void define(const std::string &name, StepFunction &&step, FinalFunction &&final) {
            ...
            auto funcPtr = new auto(
                    std::make_pair(std::forward<StepFunction>(step), std::forward<FinalFunction>(final)));
            if (int result = sqlite3_create_function_v2(
    ...
    // vector<T, A>
    template<typename T, typename A>
    inline database_binder &operator<<(database_binder &db, const std::vector<T, A> &vec) {
    ...
    template<typename T, typename A>
    inline void store_result_in_db(sqlite3_context *db, const std::vector<T, A> &vec) {
        void const *buf = reinterpret_cast<void const *>(vec.data());
        int bytes = vec.size() * sizeof(T);
    ...
    template<typename T, typename A>
    inline void get_col_from_db(database_binder &db, int inx, std::vector<T, A> &vec) {
        if (sqlite3_column_type(db._stmt.get(), inx) == SQLITE_NULL) {
            vec.clear();
        } else {
    ...
    /* for nullptr support */
    template<typename T>
    inline database_binder &operator<<(database_binder &db, const std::unique_ptr<T> &val) {
        if (val)
            db << *val;
        else
            db << nullptr;
    ...
    template<typename T>
    inline void get_col_from_db(database_binder &db, int inx, std::unique_ptr<T> &_ptr_) {
        if (sqlite3_column_type(db._stmt.get(), inx) == SQLITE_NULL) {
            _ptr_ = nullptr;
        } else {
    ...
    inline database_binder &operator<<(database_binder &db, const char(&STR)[N]) { return db << std::string(STR); }

    template<std::size_t N>
    inline database_binder &operator<<(database_binder &db, const char16_t(&STR)[N]) {
    ...
    template<typename T>
    database_binder &operator<<(database_binder &&db, const T &val) { return db << val; }
    ...
    namespace sql_function_binder {
        ...
        inline void step(
            ...
            auto ctxt = static_cast<AggregateCtxt<ContextType> *>(sqlite3_aggregate_context(db,
                                                                                            sizeof(AggregateCtxt<ContextType>)));
            if (!ctxt) return;
            try {
